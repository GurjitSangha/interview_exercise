...
describe('MessageData', () => {
  ...
  describe('delete', () => {
    ...
    it('should throw an error when trying to delete a message that does not exist', async () => {
      try {
        await messageData.delete(new ObjectID('123456789012'));
      } catch (error) {
        expect(error.message).toMatch('The message to delete does not exist');
      }
    });
  });
});
